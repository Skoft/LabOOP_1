...
void Group::AddStudent(Student* student) {
	if (Students.size() >= maxStudent) {
		cout<<"\n__________________________\nВ группе "<<numberOfGroup<<" нет места для студента "<<student->GetFirstName()<<endl;
		//нет места (cout)
	}
		...
		cout<<"\n__________________________\nУ студента "<< student->GetFirstName() <<" слишком низкий балл для группы "<< numberOfGroup <<". Неоюходимо: "<< minAverageScore<<endl;
		//плохой бал
	}
